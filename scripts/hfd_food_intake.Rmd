...
glht.table <- data.frame(Comparason = rep(names(summary(abs.nlme.glht)$test$coefficients[1:2]),2),
...
```{r food-coloric-weekly,echo=FALSE, message=FALSE}
library(plyr)
...
caloric.plot <- caloric + geom_point(aes(color=Diet))  + stat_smooth(method="loess", se=FALSE, aes(group=Diet, col=Diet)) + ylab("Food Intake (kcal/mouse/day)") + ylim(0,max(data$Caloric))
caloric.plot
```


```{r food-abs-weekly,echo=FALSE}
...
abs.plot <- c + geom_point(aes(color=Diet)) + ylim(0,max(data$Food.Per.Week)) + stat_smooth(method="loess", se=FALSE, aes(group=Diet, col=Diet)) + ylab("Food Intake (g/mouse/day)") #this is a workaround due to a bug in plyr 1.8
abs.plot
```
