...
corrplot <- function (data, xaxis, yaxis, col, title, font.x, font.y, font.size, corr.type){
  ...
  geom_text(aes(label=paste("p-value=", p.cor, sep="")), x=font.x, y=font.y-2, size=font.size) +
  xlab(xaxis) + ylab(yaxis) +
  ggtitle(title)
  #ggsave(filename)
}

### create a function that draws multiple scatterplots between the 'xaxis' and 'yaxis' variables and show 'corr.type' correlation and the p-values ####
mult.corrplot <- function (data, xaxis, yaxis, col, title, font.x, font.y, font.size, corr.type){
  ...
  p + geom_point() + facet_grid(col~.)+
      ...
      geom_text(data=p.cor, aes(label=paste("p-value=", p, sep="")), x=font.x, y=font.y-5, size=font.size) +
      xlab(xaxis) + ylab(yaxis) +
      ggtitle(title)
}
...
sub.data <- weight.wide[weight.wide$Cohort%in%c('8', '9'),]
corsFatInit <- ddply(sub.data, .(Cohort), summarise, cor=round(cor(Percent.Fat.Initial_weight, WG, method="spearman"),2))
pcorrFatInit <- ddply(sub.data, .(Cohort), summarise, p=signif(cor.test(Percent.Fat.Initial_weight, WG, method="spearman")$p.value,digits=3))
...
```
  ...
  There is no correlation between percent fat mass after fasting and the absolute weight gain, r= and p-value=.
