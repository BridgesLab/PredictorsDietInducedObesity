...
This script was most recently run on `r date()`. This script is to compare the mice body weight across different cohorts. The script also explores the relationship between Pre-fasted weight, weight loss and the weights after HFD.
...
library(xtable)
anova.pre.fasted <- aov(Weight~Cohort ,data=combined_weight[combined_weight$Fasting_stat=="Pre_fasted",])
layout(matrix(c(1,2,3,4),2,2)) # optional layout 
plot(anova.pre.fasted) # diagnostic plots
#summary(anova.init.weight)
pairwise.pre.fasted <- TukeyHSD(anova.pre.fasted)

print("Tukey's post-hoc pairwise comparisons of pre-fasted weight", comment=FALSE)
print(xtable(pairwise.pre.fasted$Cohort), comment=FALSE)
...
library(sm)
par(mfrow=c(2,2))
sm.density.compare(combined_weight$Weight[combined_weight$Fasting_stat=="Pre_fasted"], combined_weight$Cohort[combined_weight$Fasting_stat=="Pre_fasted"], xlab="Pre_fasted Body weight")
...
mult.corrplot(weight.wide, "WL", "WG", "Cohort", "Correlation between WL and WG in each cohort", 3.5, 25, 4, "kendall")
ggsave("figure/Correlation_WL_WG_each_cohort.pdf")

### all corhorts ###
corrplot(weight.wide, "WL", "WG", "Cohort", "All cohort combined (absolute values)", 3.5, 25, 5, "kendall")
...
mult.corrplot(weight.wide, "WL.pct", "WG.pct", "Cohort", "Correlation between %WL and %WG in each cohort", 12.5, 120, 4, "kendall")
...
corrplot(weight.wide, "WL.pct", "WG.pct", "Cohort", "All cohort combined (percentage)", 12.5, 120, 5, "kendall")
ggsave("figure/Correlation_percentWL_percentWG_all_cohort.pdf")

weight.5.6 <- weight.wide[weight.wide$Cohort%in%c('5', '6'),]
corrplot(weight.5.6, "WL.pct", "WG.pct", "Cohort", "Cohort 5 and 6 (percentage)", 12.5, 120, 5, "kendall")
ggsave("figure/Correlation_percentWL_percentWG_cohort5_6.pdf")

sub.weight <- weight.wide[weight.wide$WL >=0,]
corrplot(sub.weight, "WL.pct", "WG.pct", "Cohort", "No negative WL (percentage)", 12.5, 120, 5, "kendall")
ggsave("figure/Correlation_percentWL_percentWG_no_negative_WL.pdf")
...
Relationship between Pre_fasted Weight and HFD Weight
===============
```{r pre_fasted_weight_vs_WG, message=FALSE, echo=FALSE, warning=FALSE}
mult.corrplot(weight.wide, "Weight.Pre_fasted", "WG", "Cohort", "Correlation between Pre-fasted weight and WG in each cohort", 27.5, 20, 4, "kendall")
ggsave("figure/Correlation_Pre_fasted_weight_WG_each_cohort.pdf")

## All cohort combined ####
corrplot(weight.wide, "Weight.Pre_fasted", "WG", "Cohort", "Pre-fasted weight vs Weight Gain", 27.5, 20, 5, "spearman")
ggsave("figure/Correlation_Pre-fasted_Weight_WG_all_cohort.pdf")

require(RVAideMemoire)
spm.ci <- spearman.ci(weight.wide$Weight.Pre_fasted, weight.wide$WG, nrep = 1000, conf.level = 0.95)
kendall.ci(weight.wide$Weight.Pre_fasted, weight.wide$WG, B = 1000)
```
  
  There is no correlation between Pre-fasted weight and weight gain after HFD in any individual cohort. All p-values are greater than 0.05, suggesting that we cannot reject the null hypothesis that there is no correlation between init weight and weight gain. When combining all 4 cohorts together, there is a significant correlation between Pre-fasted weight and weight gain after HFD. This is likely due to the increase in sample size from about 20 mice in each cohort to about 100 mice in all. A more reliable information to look at is the 95% CI for the Spearman correlation coefficient. The 95% CI is (`r spm.ci$conf.int[[1]]`, `r spm.ci$conf.int[[2]]`). The range of this CI is wide, indicating that we don't have much knowledge about the effect and more information or more murine experiment is needed.
...
ggsave("figure/Correlation_PercentFatMassPre-fasted_PercentWG_each_cohort.pdf")
...
ggsave("figure/Correlation_PercentFatFasted_PercentWG_cohort7_9.pdf")
