...
```{r process_cohort9, message=FALSE, echo=FALSE, warning=FALSE}
...
ggplot(weight.wide, aes(x=WL, y=WG, color=Cohort, fill=Cohort)) +
  ...
  geom_smooth(method=lm, se=T) + geom_text(data=cors, aes(label=paste("r=", cor, sep="")), x=5, y=25, size=4) +
...
ggplot(weight.wide, aes(x=WL, y=WG)) +
  geom_point() + theme(legend.position="top")+
  geom_smooth(method=lm, se=T) + geom_text(aes(label=paste("r=", cor, sep="")), x=5, y=25, size=4) +
...
ggplot(weight.wide, aes(x=WL.pct, y=WG.pct, color=Cohort, fill=Cohort)) +
  geom_point() + facet_grid(Cohort~.) + theme(legend.position="top")+
  geom_smooth(method=lm, se=T) + geom_text(data=corspct, aes(label=paste("r=", cor, sep="")), x=20, y=120, size=5) +
  geom_text(data=pcorrPct, aes(label=paste("p-value=", p, sep="")), x=20, y=110, size=5) +
  ggtitle("Correlation between %WL and %WG in each cohort")
...
```
  ...
  There is a negative correlation between weight loss (or % weight loss) and weight gain (or %weight gain) after HFD in cohort 5 and 6 while the direction of the relationship reverse in cohort 8 and 9. Although the nominal p-value in cohort9 is statistically significant, it is not significant after Benjamini Hochberg adjustment. When combining all 4 cohorts together, there is a `r corpct` correlation between WL and WG and this correlation is statistically significant with p-value=`r pcorPct`. This is likely due to the increase in sample size from about 20 mice in each cohort to about 100 mice in all. A more reliable information to look at is the 95% CI for the spearman correlation coefficient. The `r print(kendall.ci(weight.wide$WL, weight.wide$WG, B = 1000))`. The range of this CI is wide, indicating that we don't have much knowledge about the effect and more information or more murine experiments are needed.  
...
ggplot(weight.wide, aes(x=Weight.Initial_weight, y=WG, color=Cohort, fill=Cohort)) +
  geom_point() + facet_grid(Cohort~.) + theme(legend.position="top")+
  geom_smooth(method=lm, se=T) + geom_text(data=corsInit, aes(label=paste("r=", cor, sep="")), x=29, y=15, size=4) +
