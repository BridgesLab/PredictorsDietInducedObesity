...
url.cohort8.group2 <- 'http://bridgeslab.uthsc.edu/mousedb/api/v1/data/?format=json&username=davebridges&api_key=fce3fde2a9a2e5dc9e04c20aad90120a621c50b3&limit=1000&offset=1000&animal__cohort=8'
...
body.weight.oneway['Lean','pval'] <- kruskal.test(Lean~Fasting_stat, data=cohort8_weight)$p.value
body.weight.oneway['Percent.Fat','pval'] <- kruskal.test(Percent.Fat~Fasting_stat, data=cohort8_weight)$p.value
...
write.csv(model_data, file='../data/processed/cohort8_training_data_for_modelling.csv')
...
library(lme4)
weight.lme <- lmer(Percent.WG~ VO2.LBM + RER + 
                     Avg.Percent.FL + Avg.Percent.LL + Light.Dark + Fast
                   + (1|Subject), data=model_data)
...
weight.rf1 <- REEMtree(Percent.WG~ VO2.LBM + RER + 
                     Avg.Percent.FL + Avg.Percent.LL + Light.Dark + Fast
...
weight.rf2 <- REEMtree(Percent.WG~ VO2.LBM + RER + 
                     Avg.Percent.FL + Avg.Percent.LL + Light.Dark + Fast
