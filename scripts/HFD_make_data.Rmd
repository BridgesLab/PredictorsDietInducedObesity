...
#list of predictors
#x = Light, y=Dark
predictors <- c('Subject', 'Avg.Percent.LL', 'Avg.Percent.FL', 
...
bestmtry <- tuneRF(merge_LD[, predictors1], merge_LD$Percent.WG,  ntreeTry=100, 
...
#The optimal number of variables for splitting is 7
rf <- randomForest(Percent.WG~., data=training[,predictors], ntree=1000, mtry=7, test=testing$Precent.WG, importance=TRUE,
...
textxy(%IncMSE, IncNodePurity, labs=names, cx = 0.5, dcol = "black", m = c(0, 0))
...
for (i in 1:k){
  ...
  mymodel <- randomForest(trainingset$Percent.WG ~ ., data = trainingset, ntree = 500, mtry=7)
  ...
  testsetCopy <- rbind(testsetCopy, as.data.frame(testset[,7]))
...
predictors1 <- c('Subject', 'Avg.Percent.LL', 'Avg.Percent.FL',
              ...
              'VO2.LBM.Bf_fasting.y',
                 'RER.Bf_fasting.x',
                'SumXYAmb.After_fasting.y','SumXYAmb.Bf_fasting.y', 'Percent.WG')

rf <- randomForest(Percent.WG~., data=training[,predictors1], ntree=1000, mtry=7, test=testing$Precent.WG, importance=TRUE,
                   keep.forest=TRUE, proximity=TRUE)
