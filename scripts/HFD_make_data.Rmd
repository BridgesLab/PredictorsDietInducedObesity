...
ggplot(cohort8_weight_wide, aes(x=After_HFD, y=diff)) +
  ...
  geom_text(aes(x = 35, y = 10, label = "Pearson R = 0.0697", parse = TRUE))
```
######Random Forest#######
cols <- c('Subject', 'Pre_fasting','After_fasting', 'VO2', 'RER', 'Light.Dark', 'SumXYAmb', 'Fast', 'After_HFD')
...
#partition the data
idx <- createDataPartition(y=model_data$After_HFD, p=0.75, list=FALSE )
training <- model_data[idx,]
testing <- model_data[-idx,]
rf <- randomForest(After_HFD ~., data=training, ntree=200)

r2 <- rSquared(testing$After_HFD, testing$After_HFD - predict(rf, testing[,cols]))

mse <- mean((testing$After_HFD - predict(rf, testing[,cols]))^2)

p <- ggplot(aes(x=actual, y=pred),
  data=data.frame(actual=testing$After_HFD, pred=predict(rf, testing[,cols])))
p + geom_point() +
