...
merge_data1 <- merge(cohort8_weight_wide, annotated_clams_data[,c(1, 11:39)], by='Subject')
merge_data1$SumXYAmb <- merge_data1$X.Ambulatory+merge_data$Y.Ambulatory
...
merge_data1$Fast <- ifelse(merge_data1$Interval<start_fasting_interval,"Bf_fasting",ifelse(merge_data1$Interval>=stop_fasting_interval, "After_fasting", "During_fasting"))

###Remove outliers:
merge_data <- merge_data1[! merge_data1$RER %in% c('-5369.972', '-672.8121', '-335.7452', '-139.4203'), ]
merge_data <- merge_data1[! merge_data1$RER < -40,]
...
combine.data <- rbind(model_data[ ,c(1:8, 10)], cohort9_model_data[, 2:10])
