...
```{r weight-loss, include=FALSE, echo=FALSE, dev=c('png','pdf')}
...
```{r weight-loss-pct, include=FALSE, echo=FALSE, dev=c('png','pdf')}
...
```{r linear-modeling-gain, echo=FALSE}
#calculated correlation coefficients
...
Alternatively we generated a model where percent weight gain is dependent only on the treatment.  We then performed a F-test to see if inclusion of the pre-diet improved the model.  In this case it does, with a p-value of **`r round(model.improvement,9)`**.
