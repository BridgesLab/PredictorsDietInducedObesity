...
The raw data is in a file named `r data_file` and are mapped against the file in `r mapping_file`.  The mouse level data is in Table below:
...
anova.summary.no.int <- data.frame(row.names=hormones.tested,
     Feeding = rep(NA, length(hormones.tested)),
     Treatment = rep(NA, length(hormones.tested)))
