...
genes.of.interest <- c("Retn","Pparg","Lep","Hsd11b1","Pnpla2","Tbc1d9b","Irf4", "Clk4","Slc1a3","Ap2s1","Rer1","Ywhae","Eif4ebp3","Hsd11b1")
...
#get a table of just the genes of interest
genes.of.interest.data.quad <- rpkm.data.unique.quad[genes.of.interest,]
#put the data in long format
library(tidyr)
library(dplyr)
genes.of.interest.long.quad <- 
  ...
  select(-gene_biotype) %>%
  gather(SampleID, expression, -external_gene_name)
goi.long.quad <- merge(mapping, genes.of.interest.long.quad, by="SampleID")
...
for (gene in genes.of.interest) {
  ...
  gene.data.quad <- goi.long.cal.quad[goi.long.cal.quad$external_gene_name==gene,]
  ...
  p + geom_bar(position=dodge, stat="identity") + geom_errorbar(aes(ymin=Average-SE, ymax=Average+SE), position=dodge, width=0.1) + 
   xlab("") + ylab("mRNA Expression (RPKM)")+ theme_bw() + ggtitle(gene) + theme(panel.grid.minor = element_blank()) + theme(panel.grid.major = element_blank()) + theme(panel.border=element_blank()) + scale_x_discrete(labels=gene.data$Diet) + theme(axis.line = element_line(color = 'black'))+scale_colour_grey(start = 0.5, end = .9)
ggsave(filename=paste('figures/barplots/Adipose-',gene,' - barplot-HTSeq.pdf',sep=""))
  ...
  q + geom_bar(position=dodge, stat="identity") + geom_errorbar(aes(ymin=Average-SE, ymax=Average+SE), position=dodge, width=0.1) + 
   xlab("") + ylab("mRNA Expression (RPKM)")+ theme_bw() + ggtitle(gene) + 
