...
transcripts.of.interest <- as.character(droplevels(subset(annotation, external_gene_name %in% genes.of.interest)$ensembl_transcript_id))

#get a table of just the genes of interest
genes.of.interest.data <- rpkm.data.unique[transcripts.of.interest,]
...
#get a table of just the genes of interest
genes.of.interest.data.quad <- rpkm.data.unique.quad[transcripts.of.interest,]
#put the data in long format
genes.of.interest.long.quad <- 
  genes.of.interest.data.quad %>%
  select(-gene_biotype, -ensembl_gene_id) %>%
  gather(SampleID, expression, -external_gene_name, -ensembl_transcript_id)
goi.long.quad <- droplevels(merge(mapping, genes.of.interest.long.quad, by.x="Barcode", by.y="SampleID"))
...
library(ggplot2)
for (transcript in goi.long.cal$ensembl_transcript_id[!(is.na(goi.long.cal$ensembl_transcript_id))]) {
  gene.data.adipose <- subset(goi.long.cal, ensembl_transcript_id==transcript)
  gene.data.quad <- subset(goi.long.cal.quad, ensembl_transcript_id==transcript)
  ...
  p + geom_bar(position=dodge, stat="identity") +
     ...
     theme_bw() + ggtitle(paste(as.character(gene.data.adipose$external_gene_name[1]), transcript, sep=" - ")) + 
  ...
  q + geom_bar(position=dodge, stat="identity") +
     geom_errorbar(aes(ymin=Average-SE, ymax=Average+SE), position=dodge, width=0.1) +
     xlab("") + ylab("mRNA Expression (RPKM)")+ 
     theme_bw() + ggtitle(paste(as.character(gene.data.quad$external_gene_name[1]), transcript, sep=" - ")) + 
     theme(panel.grid.minor = element_blank()) + theme(panel.grid.major = element_blank()) + 
