...
transcripts.of.interest <- droplevels(subset(annotation, external_gene_name %in% genes.of.interest)$ensembl_transcript_id)
...
```{r barplots}
library(ggplot2)
for (gene in levels(goi.long.cal$external_gene_name)) {
  #pdf(sprintf('../figures/%s-barplot.pdf', gene))
  gene.data <- subset(goi.long.cal, external_gene_name==gene)
  ...
  ggsave(filename=paste('figures/barplots/Adipose-',gene,'-barplot-kallisto.pdf',sep=""))
}
