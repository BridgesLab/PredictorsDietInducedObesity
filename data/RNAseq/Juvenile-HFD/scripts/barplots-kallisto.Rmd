...
genes.of.interest <- c("Retn","Pparg","Lep","Hsd11b1","Pnpla2","Tbc1d9b","Clk4","Slc1a3","Ap2s1","Rer1","Ywhae","Eif4ebp3","Hsd11b1")
...
for (transcript in goi.long.cal$ensembl_transcript_id[!(is.na(goi.long.cal$ensembl_transcript_id))]) {
  gene.data <- subset(goi.long.cal, ensembl_transcript_id==transcript)
  ...
  p + xlab("") + ylab("mRNA Expression (RPKM)")+ theme_bw() + ggtitle(paste(as.character(gene.data$external_gene_name[1]), transcript, sep=" - ")) + theme(panel.grid.minor = element_blank()) + theme(panel.grid.major = element_blank()) + theme(panel.border=element_blank()) + scale_x_discrete(labels=gene.data$Diet) + theme(axis.line = element_line(color = 'black'))+scale_colour_grey(start = 0.5, end = .9)
  ggsave(filename=paste('figures/barplots/Adipose-',
                        paste(as.character(gene.data$external_gene_name[1]), transcript, sep=" - ")
